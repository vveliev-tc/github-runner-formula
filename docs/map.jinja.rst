...
We will see a quick setup to configure the ``github-runner`` formula for different DNS domain names and several roles.
...
Create per DNS configuration for ``github-runner`` formula
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Now, we can provides the per DNS domain name configuration files for the ``github-runner`` formulas under ``/srv/salt/github-runner/parameters/``.
...
.. code-block:: console
    ...
    mkdir -p /srv/salt/github-runner/parameters/dns:domain/
    ln -s dns:domain /srv/salt/github-runner/parameters/domain

We create a configuration for the DNS domain ``example.net`` in ``/srv/salt/github-runner/parameters/dns:domain/example.net.yaml``:
...
We create another configuration for the DNS domain ``example.com`` in the Jinja YAML template ``/srv/salt/github-runner/parameters/dns:domain/example.com.yaml.jinja``:
...
Create per role configuration for ``github-runner`` formula
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Now, we can provides the per role configuration files for the ``github-runner`` formulas under ``/srv/salt/github-runner/parameters/``.

We create the directory for roles:

.. code-block:: console

    mkdir -p /srv/salt/github-runner/parameters/roles
...
We create a configuration for the role ``github-runner/server`` in ``/srv/salt/github-runner/parameters/roles/github-runner/server.yaml``:
...
We create another configuration for the role ``github-runner/client`` in ``/srv/salt/github-runner/parameters/roles/github-runner/client.yaml``:
...
.. code-block:: console
    ...
    salt 'server-*' grains.append roles github-runner/server

For the clients:

.. code-block:: console

    salt 'client-*' grains.append roles github-runner/client
...
.. code-block:: console
    ...
    mv /srv/salt/github-runner/parameters/dns:domain/  '/srv/salt/github-runner/parameters/dns!domain/'
...
.. code-block:: sls
    ...
    {%- from tplroot ~ "/map.jinja" import mapdata as github-runner with context %}

    test-does-nothing-but-display-github-runner-as-json:
      test.nop:
        - name: {{ github-runner | json }}
...
Here is an example based on `template-formula/github-runner/config/file.sls`_:
...
.. code-block:: sls
    ...
    github-runner-config-file-file-managed:
      file.managed:
        ...
        - mode: 644
        - user: root
        - group: {{ github-runner.rootgroup }}
        - makedirs: True
        - template: jinja
        - require:
          - sls: {{ sls_package_install }}
        - context:
            github-runner: {{ github-runner | json }}

This ``sls`` file expose a ``github-runner`` context variable to the jinja template which could be used like this:
...
.. code-block:: jinja
    ...
    winner of the merge: {{ github-runner['winner'] }}
...
.. _template-formula/github-runner/config/file.sls: https://github.com/saltstack-formulas/template-formula/blob/master/github-runner/config/file.sls
